...
class SkipGramDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ############################################################
        input_token_id= random.randint(0, len(sample_tokens) - 1)
        target_token=sample_tokens[input_token_id]
        input_id=self.vocab[target_token] if target_token in self.vocab else self.vocab['<UNK>']
        context = []
        for i in range(max(0, input_token_id - self.context_size), min(len(sample_tokens), input_token_id + self.context_size+1)):
            if i !=input_token_id:
                context.append(sample_tokens[i])
        if len(context) == 0:
            return self.vocab['<UNK>'], self.vocab['<UNK>']
        context_token_id= random.randint(0, len(context) - 1)
        context_token= context[context_token_id]
        context_id=self.vocab[context_token] if context_token in self.vocab else self.vocab['<UNK>']
        ############################################################
...
def build_vocab(train_dataset, vocab_size=5000):
    ...
    ############################################################
    vocab = {}
    tokens = tokenize(train_dataset['text'])
    count={}
    for word in tokens:
    ...
    sorted={token[i]:counts[i] for i in idx}
    words=list(sorted.keys())
    i=0
    while i<vocab_size:
        vocab[words[i]]=i
        i+=1
